...
- A snapshot becomes visible only once it is complete. A snapshot that fails is removed again at once; what a killed `juicefs snapshot create` left behind is removed by [`juicefs gc --delete`](../reference/command_reference.mdx#gc) once it is more than a day old.
- By default, a snapshot is published only once JuiceFS has validated that the tree did not change while it was copied: the tree is copied, the copy is compared with the live tree, and the snapshot is published only if they match. If anything changed, the copy is discarded and tried again: the snapshot is tried up to three times, after which `juicefs snapshot create` fails with "device or resource busy" rather than publish a mixed tree. Take the snapshot when the tree is quieter, or stop writing to it while it is taken.
...
Snapshots are not counted against the capacity or inode limits of the volume, nor against directory, user or group quotas, and `df` does not include them: snapshot metadata bypasses the inode and quota accounting altogether, so none of those limits stops a snapshot from being taken. Their usage is reported by `juicefs snapshot list` instead, as the number of inodes and the size of each snapshot. That size is counted as [`juicefs summary`](../reference/command_reference.mdx#summary) counts a directory, with every file rounded up to 4 KiB and 4 KiB per directory, so it is what the same tree costs against the capacity when it is live. Since the files of a snapshot share their blocks with the originals and with other snapshots, it is not the space that deleting the snapshot would free.

While a snapshot exists, the blocks it references stay in object storage even after the original files are deleted. `juicefs snapshot delete` removes the metadata of the snapshot and deletes the blocks that no other file or snapshot references from the object storage before it returns, so like [`juicefs gc`](../reference/command_reference.mdx#gc) it needs access to the object storage: the credentials stored in the volume, and the `JFS_RSA_PASSPHRASE` environment variable when the private key of an encrypted volume has a passphrase. If the object storage refuses some of the deletes, the command reports it and exits with an error; the snapshot is gone all the same, and the blocks it left behind are reclaimed by a later `juicefs gc --delete`.
