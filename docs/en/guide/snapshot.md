...
- The first attempt to create a snapshot raises the minimum client version of the volume to 1.5, since older clients cannot understand snapshots: from then on they can no longer mount the volume or run `juicefs gc` on it. The attempt raises it before anything else, so it stays raised even if that snapshot then fails, for example because an older client is still mounted: this way no older client can mount while the ones already mounted are being stopped. The client logs a warning when it raises the minimum version. The minimum version stays raised for good, even once every snapshot is deleted, since [`juicefs config`](../reference/command_reference.mdx#config) never lowers it.
...
- [`juicefs dump`](../reference/command_reference.mdx#dump) and [`juicefs load`](../reference/command_reference.mdx#load) keep snapshots, in both the JSON and the binary format. A dump records the minimum client version the volume has when it is taken, even if the dumping client loaded an older one, and fails if the first snapshot is created while it runs; dump again then. A dump of a volume that has held snapshots therefore always carries a minimum client version of 1.5, and must be loaded with a 1.5 or later client: it checks the minimum client version of a dump before writing anything, and refuses a dump it is too old for. Clients older than 1.5 do not make that check: they load everything but the snapshots, then fail, leaving an incomplete volume behind.
