...
|`--max-snapshots=0` <VersionAdd>1.5</VersionAdd> |Limit the number of [snapshots](../guide/snapshot.md) the volume may hold, default to 0 which means no limit.|
...
|`--max-snapshots value` <VersionAdd>1.5</VersionAdd> |limit for number of [snapshots](../guide/snapshot.md), 0 means no limit|
...
|`--hide-internal` <VersionAdd>1.4</VersionAdd> |hide all internal files (`.accesslog`, `.stats`, `.config`, `.control`) and the `.trash` and `.snapshots` directories from directory listings, they are still accessible via their full path (default: false)|
...
|`--prefix-internal` <VersionAdd>1.1</VersionAdd> |add '.jfs' prefix to all internal files, and to the `.trash` and `.snapshots` directories (default: false)|
...
### `juicefs snapshot` <VersionAdd>1.5</VersionAdd> {#snapshot}

Create, list, hold and delete read-only snapshots of directories. A snapshot copies metadata only and is kept under `/.snapshots`. Read [Snapshots](../guide/snapshot.md) for more.

`snapshot delete` deletes the blocks only that snapshot referenced from the object storage, so like [`juicefs gc`](#gc) it needs access to it: the credentials stored in the volume, and the `JFS_RSA_PASSPHRASE` environment variable when the private key of an encrypted volume has a passphrase. If some of those deletes fail, the command reports it and exits with an error, and a later `juicefs gc --delete` reclaims the blocks.

#### Synopsis

```shell
juicefs snapshot create [command options] META-URL
juicefs snapshot list META-URL
juicefs snapshot delete [command options] META-URL
juicefs snapshot hold [command options] META-URL
juicefs snapshot release [command options] META-URL

# Snapshot a directory
juicefs snapshot create redis://localhost --path /data --name before-upgrade

# List snapshots with the inodes and size each one holds
juicefs snapshot list redis://localhost

# Delete a snapshot and release the data only it referenced
juicefs snapshot delete redis://localhost --name before-upgrade

# Keep a snapshot from being deleted while a backup reads it, then let it go
...
|Items|Description|
|-|-|
|`--path value`|directory to snapshot, relative to the root of the volume (default: `/`); it cannot be in a snapshot (`create` only)|
|`--name value`|name of the snapshot, at most 255 bytes without control characters (`create`, `delete`, `hold` and `release`)|
|`--tag value`|name of the hold, 1 to 200 bytes of UTF-8 without control characters (`hold` and `release` only)|
|`--best-effort`|keep the copy even if the tree changes while it is taken, so it may not be a point-in-time image (default: false) (`create` only)|

Each subcommand accepts its own options only.
