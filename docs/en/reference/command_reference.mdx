...
|Items|Description|
|-|-|
|`--path value`|directory to snapshot, relative to the root of the volume (default: `/`); it cannot be in a snapshot, used by `create`|
|`--name value`|name of the snapshot, used by `create`, `delete`, `hold` and `release`|
|`--tag value`|name of the hold, used by `hold` and `release`: 1 to 200 bytes of UTF-8 without control characters|
|`--best-effort`|keep the copy even if the tree changes while it is taken, so it may not be a point-in-time image (default: false), used by `create`|
